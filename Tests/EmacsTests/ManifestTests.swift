import Foundation
import Testing

@testable import Emacs

@Suite("Plugin Manifest")
struct ManifestTests {

  private enum ManifestError: Error {
    case entryPointFailed
    case nilManifest
    case invalidJSON
  }

  private func loadManifest() throws -> [String: Any] {
    guard let apiPtr = osaurus_plugin_entry() else {
      throw ManifestError.entryPointFailed
    }

    let fnPtrSize = MemoryLayout<UnsafeRawPointer?>.stride
    let initPtr = apiPtr.load(
      fromByteOffset: fnPtrSize,
      as: (@convention(c) () -> UnsafeMutableRawPointer?).self)
    let ctx = initPtr()

    let getManifestPtr = apiPtr.load(
      fromByteOffset: fnPtrSize * 3,
      as: (@convention(c) (UnsafeMutableRawPointer?) -> UnsafePointer<CChar>?).self)
    guard let cStr = getManifestPtr(ctx) else {
      throw ManifestError.nilManifest
    }
    let jsonString = String(cString: cStr)

    let freeStringPtr = apiPtr.load(
      fromByteOffset: 0,
      as: (@convention(c) (UnsafePointer<CChar>?) -> Void).self)
    freeStringPtr(cStr)

    let destroyPtr = apiPtr.load(
      fromByteOffset: fnPtrSize * 2,
      as: (@convention(c) (UnsafeMutableRawPointer?) -> Void).self)
    destroyPtr(ctx)

    guard let data = jsonString.data(using: .utf8),
      let manifest = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
    else {
      throw ManifestError.invalidJSON
    }
    return manifest
  }

  private func toolMap(from manifest: [String: Any]) -> [String: [String: Any]] {
    let capabilities = manifest["capabilities"] as? [String: Any]
    let tools = capabilities?["tools"] as? [[String: Any]] ?? []
    return Dictionary(
      uniqueKeysWithValues: tools.compactMap { tool -> (String, [String: Any])? in
        guard let id = tool["id"] as? String else { return nil }
        return (id, tool)
      })
  }

  @Test("manifest has correct plugin identity")
  func pluginIdentity() throws {
    let manifest = try loadManifest()
    #expect(manifest["plugin_id"] as? String == "osaurus.emacs")
    #expect(manifest["name"] as? String == "Emacs")
  }

  @Test("manifest declares one approval-gated Lisp execution tool")
  func toolContract() throws {
    let map = try toolMap(from: loadManifest())
    #expect(Set(map.keys) == ["execute_emacs_lisp_code"])

    let tool = try #require(map["execute_emacs_lisp_code"])
    #expect(tool["permission_policy"] as? String == "ask")
    #expect(tool["requirements"] as? [String] == [])

    let params = tool["parameters"] as? [String: Any]
    let required = params?["required"] as? [String] ?? []
    #expect(required == ["code"])
  }
}

@Suite("Tool Invocation")
struct ToolInvocationTests {

  private func loadAPI() -> (
    freeString: (@convention(c) (UnsafePointer<CChar>?) -> Void),
    initContext: (@convention(c) () -> UnsafeMutableRawPointer?),
    destroy: (@convention(c) (UnsafeMutableRawPointer?) -> Void),
    invoke: (@convention(c) (
      UnsafeMutableRawPointer?, UnsafePointer<CChar>?, UnsafePointer<CChar>?, UnsafePointer<CChar>?
    ) -> UnsafePointer<CChar>?)
  ) {
    let apiPtr = osaurus_plugin_entry()!
    let fnPtrSize = MemoryLayout<UnsafeRawPointer?>.stride
    return (
      apiPtr.load(fromByteOffset: 0, as: (@convention(c) (UnsafePointer<CChar>?) -> Void).self),
      apiPtr.load(fromByteOffset: fnPtrSize, as: (@convention(c) () -> UnsafeMutableRawPointer?).self),
      apiPtr.load(
        fromByteOffset: fnPtrSize * 2,
        as: (@convention(c) (UnsafeMutableRawPointer?) -> Void).self),
      apiPtr.load(
        fromByteOffset: fnPtrSize * 4,
        as: (@convention(c) (
          UnsafeMutableRawPointer?, UnsafePointer<CChar>?, UnsafePointer<CChar>?,
          UnsafePointer<CChar>?
        ) -> UnsafePointer<CChar>?).self)
    )
  }

  @Test("missing executable path returns structured error")
  func missingExecutablePathError() throws {
    let api = loadAPI()
    let ctx = api.initContext()
    defer { api.destroy(ctx) }

    let payload = #"{"code":"(+ 1 2)","emacsclient_path":"/not/a/real/emacsclient"}"#
    let resultPtr = payload.withCString { payloadPtr in
      "execute_emacs_lisp_code".withCString { idPtr in
        "tool".withCString { typePtr in
          api.invoke(ctx, typePtr, idPtr, payloadPtr)
        }
      }
    }
    let cStr = try #require(resultPtr)
    let result = String(cString: cStr)
    api.freeString(cStr)

    #expect(result.contains(#""error""#))
    #expect(result.contains("Failed to execute emacsclient"))
  }
}
